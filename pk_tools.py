...
def getpkmultipoles(datgrid,w,W,nx,ny,nz,lx,ly,lz,kbins):
    '''
    Obtain the auto-correlation multipoles.
    ...
    p0,p2,p4: monopole, quadrupole, hexadecapole
    nmodes: number of modes in each k bin
    ...
    nc = nx*ny*nz
    pkspec = getpkspec(datgrid,nc,vol,w,W)
    p0,p2,p4,k,nmodes = binpole(pkspec,nx,ny,nz,lx,ly,lz,kbins,FullPk=False,doindep=True)
    return p0,p2,p4,k,nmodes
...
def binpole(pkspec,nx,ny,nz,lx,ly,lz,kbins,k_FG=0,FullPk=False,doindep=True):
    '''
    Bin 3D power spectrum in multipoles.
    '''
    #print('\nBinning 3D power spectrum into monopole, quadrupole, hexadecapole...')
    kspec,muspec,indep = getkspec(nx,ny,nz,lx,ly,lz,FullPk)
    if doindep==True:
        pkspec = pkspec[indep==True]
        kspec = kspec[indep==True]
